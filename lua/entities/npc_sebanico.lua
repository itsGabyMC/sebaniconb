...
util.PrecacheSound("sebanico/panic.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_sebanico_acquire_distance = CreateConVar("npc_sebanico_acquire_distance", 2500, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which sebanico will chase a target.")

local npc_sebanico_spawn_protect = CreateConVar("npc_sebanico_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, sebanico will not target players or hide within 200 units of a spawn point.")

local npc_sebanico_attack_distance = CreateConVar("npc_sebanico_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of sebanico's attack.")

local npc_sebanico_attack_interval = CreateConVar("npc_sebanico_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between sebanico's attacks.")

local npc_sebanico_attack_force = CreateConVar("npc_sebanico_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of sebanico's attack. Higher values throw things farther.")

local npc_sebanico_smash_props = CreateConVar("npc_sebanico_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, sebanico will punch through any props placed in their way.")

local npc_sebanico_hiding_scan_interval = CreateConVar("npc_sebanico_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "sebanico will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant sebanicos are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local npc_sebanico_hiding_repath_interval = CreateConVar("npc_sebanico_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to sebanico's hiding spot will be redetermined every X seconds.")

local npc_sebanico_chase_repath_interval = CreateConVar("npc_sebanico_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of sebanico's target will be redetermined every X seconds.")

local npc_sebanico_expensive_scan_interval = CreateConVar("npc_sebanico_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local npc_sebanico_force_download = CreateConVar("npc_sebanico_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download sebanico resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear sebanico!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (npc_sebanico_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("sebanico_nag")
util.AddNetworkString("sebanico_nagresponse")
util.AddNetworkString("sebanico_navgen")
...
local VECTOR_sebanico_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_sebanico_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_sebanico: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other sebanicos, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC() and
	        ent:Health() > 0 and
	        class ~= "npc_sebanico" and
	        not class:find("bullseye"))
end

--HACK!!! Because this is an issue a lot of people keep asking me about.
hook.Add("PlayerSpawnedNPC", "sebanicoMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_sebanico") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.sebanico_HasBeenNagged) then return end
	ply.sebanico_HasBeenNagged = true

	-- Try to explain why sebanico isn't working.
	net.Start("sebanico_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("npc_sebanico: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_sebanico: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("sebanico_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.sebanico_HasBeenNagged = false
		return
	end
	...
	for _, sebanico in pairs(ents.FindByClass("npc_sebanico")) do
		sebanico:Remove()
	end

	print("npc_sebanico: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "sebanicoNavGen", navEndGenerate)
	else
		print("npc_sebanico: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_sebanico_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `npc_sebanico_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (npc_sebanico_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_sebanico_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. sebanico means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * npc_sebanico_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * npc_sebanico_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not npc_sebanico_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_sebanico_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_sebanico_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "sebanico/spring.wav" or "sebanico/jump.wav"), 350, 100)
	end
end
...
local tauntSounds = {
	"sebanico/tooslow.wav",
	"sebanico/stepitup.wav",
	"sebanico/tooeasy.wav",
	"sebanico/pieceofcake.wav"
}
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > npc_sebanico_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > npc_sebanico_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(npc_sebanico_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > npc_sebanico_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= npc_sebanico_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= npc_sebanico_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= npc_sebanico_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= npc_sebanico_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local sebanicosLastFrame = 0
hook.Add("Tick", "sebanicoBenchmark", function()
	local now = SysTime()
	local numsebanicos = #ents.FindByClass("npc_sebanico")

	if (timeAll ~= 0) then

		local sebanicoTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    sebanicoTime / sebanicosLastFrame, sebanicoTime, frameTime, (sebanicoTime / frameTime) * 100, sebanicosLastFrame, numThink, (numThink / sebanicosLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	sebanicosLastFrame = numsebanicos
end)]]
...
killicon.Add("npc_sebanico", "sebanico/npc_sebanico_killicon", color_white)
language.Add("npc_sebanico", "sebanico")
...
local function DevPrint(devLevel, msg)
	if (developer:GetInt() >= devLevel) then
		print("npc_sebanico: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a sebanico.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_sebanico_music_volume = CreateConVar("npc_sebanico_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by sebanico. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another sebanico comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, sebanicos do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 400 -- Max volume is achieved when MUSIC_sebanico_PANIC_COUNT sebanicos are this close, or an equivalent score.
local MUSIC_sebanico_PANIC_COUNT = 8 -- That's a lot of sebanico.

local MUSIC_sebanico_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_sebanico_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("npc_sebanico") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("sebanicoPanicMusicUpdate")
	...
	if (panicMusic == nil) then
		if (IsValid(LocalPlayer())) then
			panicMusic = CreateSound(LocalPlayer(), "sebanico/panic.mp3")
			panicMusic:Stop()
		else
	...
	if (npc_sebanico_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		panicMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "npc_sebanico") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_sebanico_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear sebanico taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(npc_sebanico_music_volume:GetFloat(), 0, 1))
...
local function startTimer()
	if (not timer.Exists("sebanicoPanicMusicUpdate")) then
		timer.Create("sebanicoPanicMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local sebanicoMaterial = Material("sebanico/npc_sebanico.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(sebanicoMaterial)
...
hook.Add("OnEntityCreated", "sebanicoInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_sebanico") then return end

	local sebanicoEntTable = scripted_ents.GetStored("npc_sebanico")

	table.Merge(ent, sebanicoEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("sebanico_removed", sebanicoDeregister)
end)

hook.Add("NetworkEntityCreated", "sebanicoNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_sebanico") then return end
...
surface.CreateFont("sebanicoHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("sebanicoHUDSmall", {
	font = "Arial",
	size = 24
})
...
local flavourTexts = {
	{
		"Gotta learn fast!",
		"Learning this'll be a piece of gake!",
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("sebanico is studying this map.", "sebanicoHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "sebanicoHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "sebanicoHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "sebanicoHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "sebanicoHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("sebanico_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "sebanicoNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. sebanico doesn't even know where to start with this map.
...
local function navGenerate()
	net.Start("sebanico_nagresponse")
		net.WriteBit(true)
	net.SendToServer()
end
...
local function navWarning()
	Derma_Query([[It will take a while for sebanico to figure this map out.
...
-- Lazy
net.Receive("sebanico_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! sebanico doesn't know this map.
Would you like sebanico to learn it?]],
		            "This map is currently not sebanico-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! sebanico doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to sebanico.]],
		            "This map is currently not sebanico-compatible!",
		            "Ok", nagAgain,
